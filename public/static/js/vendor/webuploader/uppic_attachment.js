function productImageInit(url,group_id,$picType,token,a_type) {
	var $wrap = $('#uploader'),
	// 图片容器
	...
	if (!WebUploader.Uploader.support()) {
		artAlert('Web Uploader 不支持您的浏览器！如果你使用的是IE浏览器，请尝试升级 flash 播放器');
		throw new Error(
	...
	uploader = WebUploader.create({
		...
		// swf文件路径
		swf : 'public/js/vendor/webuploader/Uploader.swf',
		disableGlobalDnd : true,
		formData:{attachment_group_id:group_id,_token:token,a_type:a_type},
		chunked : true,
		server : url,
		fileNumLimit : 20,
		fileSizeLimit : 20 * 10 * 1024 * 1024,
		...
		if (state === 'ready') {
			...
			if (stats.uploadFailNum) {
				text = '已成功上传'
						...
						+ '张照片，'
						+ stats.uploadFailNum
						+ '张照片上传失败，<a class="retry" href="javascript:">重新上传</a>失败图片或<a class="ignore" href="javascript:">忽略</a>'
			}
		} else {
			stats = uploader.getStats();
	...
	uploader.onUploadStart=function(file){
		var picType=1;
		$picType.each(function(){
			if($(this).is(':checked')){
				picType=$(this).val();
			}
		});
		uploader.option('formData',{pic_type:picType});
	};
